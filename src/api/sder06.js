...
//返回所有的人员信息
export let listPlan = async(params)=>{
    let data = await $getData('Sder06',params)
   if(data){
     $msg_success('查询成功！')
 ...
 export let addForm = async(params)=>{
    ...
    let data= await $post('Sder06',params)
    if(data){
        $msg_success('新增成功')
    }
    return data
...
export let addForm2 = async(params)=>{
  ...
  let data= await $post('Sder02',params)
  if(data){
      $msg_success('新增成功')
  }
  return data
...
export let updateForm = async(params)=>{
    ...
    //     return false
    // }
    let data= await $put(`Sder01/${params.id}`,params)
    if(data){
        $msg_success('修改成功')
    }
    return data
 ...
 export let updateForm2 = async(params)=>{
    ...
    //     return false
    // }
    let data= await $put(`Sder02/${params.id}`,params)
    if(data){
        $msg_success('修改成功')
    }
    return data
